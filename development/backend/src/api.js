...
const getLinkedUser = async (headers) => {
  ...
  mylog(target);
  const qs = `select * from session where value = ?`;//done
...
const allActive = async (req, res) => {
  ...
  const searchRecordQs = `select * from record where status = "open" order by updated_at desc, record_id asc limit ? offset ?`;//doing
  ...
  const searchUserQs = 'select * from user where user_id = ?';//no idx
  const searchGroupQs = 'select * from group_info where group_id = ?';//no idx
  const searchThumbQs =
    'select * from record_item_file where linked_record_id = ? order by item_id asc limit 1';
  const countQs = 'select count(*) from record_comment where linked_record_id = ?';
  const searchLastQs = 'select * from record_last_access where user_id = ? and record_id = ?';
